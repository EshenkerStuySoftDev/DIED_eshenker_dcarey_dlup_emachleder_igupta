...
def createTables():
    ...
    c = db.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS minesweeper (rank INTEGER, player TEXT, scores INTEGER);""")
    c.execute("""CREATE TABLE IF NOT EXISTS pacman (rank INTEGER, player TEXT, scores INTEGER);""")
    c.execute("""CREATE TABLE IF NOT EXISTS whackamole (rank INTEGER, player TEXT, scores INTEGER);""")
    c.execute("""CREATE TABLE IF NOT EXISTS snake (rank INTEGER, player TEXT, scores INTEGER);""")
    c.execute("""CREATE TABLE IF NOT EXISTS brick (rank INTEGER, player TEXT, scores INTEGER);""")
    db.commit()
    db.close()
...
def initialize(game, rank):
    ...
    db.text_factory = text_factory
    c = db.cursor()
    command = "INSERT INTO ? (rank, player, scores) VALUES (?,?,0);"
    player = "-"
    c.execute(command, (game, rank, player))
    db.commit()
    db.close()
